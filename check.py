...
def equal(outcome, expected, message: str = '') -> None:
    ...
    if outcome_type != expected_type or outcome != expected:
        ...
        if outcome_type != expected_type:
            ...
            print("failure: outcome, {0}, differs from expected value, {1}".format(
                repr(outcome), repr(expected)))
        print('----')
...
def within(outcome: float, expected: float, epsilon: float, message: str = '') -> None:
    ...
    types_ok = (outcome_type == float and expected_type in (int, float)
                or expected_type == float and outcome_type in (int, float))
...
def summary(clear_counters: bool = False) -> None:
    ...
    print("Ran {0} tests, {1} failed".format(
        _num_tests_run, _num_tests_failed))
    if clear_counters:
        _num_tests_run = 0
        _num_tests_failed = 0
...
def _print_failed_check_expression():
    ...
    try:
        ...
        print("location: {0}, line {1}".format(
            os.path.basename(filename), lineno), end='')
